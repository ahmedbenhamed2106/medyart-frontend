...
export default function Home() {
  ...
  // Exclusive Publisher Restriction: Only "Mehdi" or "mehdi" is authorized
  const isOwner = user && user.toLowerCase() === 'mehdi';

  return (
    <div className={darkMode ? 'dark' : ''}>
      <div className="min-h-screen bg-neutral-100 dark:bg-neutral-950 text-neutral-900 dark:text-neutral-100 p-6 md:p-12 font-sans transition-colors duration-300">
        
        {/* Header with Colorful Vivid Logo */}
        <header className="flex flex-wrap justify-between items-center mb-12 border-b border-neutral-200 dark:border-neutral-800 pb-6 gap-4">
          ...
          <div className="flex items-center gap-3">
            <button 
              ...
              className="p-2.5 rounded-xl bg-neutral-200 dark:bg-neutral-900 text-xs font-bold hover:opacity-80 transition-opacity border border-neutral-300 dark:border-neutral-800"
            >
            ...
            {user ? (
              <div className="flex items-center gap-2">
                ...
                <button 
                  ...
                  className="bg-neutral-200 dark:bg-neutral-900 text-xs font-bold px-4 py-2.5 rounded-xl border border-neutral-300 dark:border-neutral-800 transition-all"
                >
                  Logout ({user})
                </button>
              </div>
        ...
        <main className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {photos.length === 0 ? (
            <div className="col-span-full text-center py-20 bg-white dark:bg-neutral-900/50 border border-neutral-200 dark:border-neutral-800 rounded-3xl backdrop-blur-sm">
              <p className="text-neutral-500 mb-4 font-medium">No artwork uploaded to MedyArt gallery yet.</p>
              {isOwner ? (
                <button 
                  onClick={() => setShowUpload(true)}
            ...
            photos.map((photo) => (
              <div 
                key={photo.id} 
                className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-2xl overflow-hidden shadow-lg hover:border-pink-500/50 transition-all cursor-pointer group"
